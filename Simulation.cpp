...
RatData generateSimulationMLE(RatData& ratdata, MazeGraph& suboptimalHybrid3, MazeGraph& optimalHybrid3, std::map<std::string, std::vector<double>> clusterParams, RInside &R)
{
    ...
    while(!endLoop)
    {
        auto aca2_Suboptimal_Hybrid3_cpy = std::make_shared<Strategy>(suboptimalHybrid3,"aca2", alpha_aca_subOptimal, gamma_aca_subOptimal, 0, crpAlpha, phi, eta, false);
        auto aca2_Optimal_Hybrid3_cpy = std::make_shared<Strategy>(optimalHybrid3,"aca2",alpha_aca_optimal, gamma_aca_optimal, 0, crpAlpha, phi, eta, true);
        
        auto drl_Suboptimal_Hybrid3_cpy = std::make_shared<Strategy>(suboptimalHybrid3,"drl", alpha_drl_subOptimal, beta_drl_subOptimal, lambda_drl_subOptimal, crpAlpha, phi, eta, false);
        auto drl_Optimal_Hybrid3_cpy = std::make_shared<Strategy>(optimalHybrid3,"drl",alpha_drl_optimal, beta_drl_optimal, lambda_drl_optimal, crpAlpha, phi, eta, true);

        std::vector<double> s0rewards = {0,0,0,0,0,0,0,5,0};
        std::vector<double> s1rewards = {0,0,0,0,0,0,0,0,5};

        aca2_Optimal_Hybrid3_cpy->setRewardsS0(s0rewards);
        aca2_Optimal_Hybrid3_cpy->setRewardsS1(s1rewards);

        drl_Optimal_Hybrid3_cpy->setRewardsS0(s0rewards);
        drl_Optimal_Hybrid3_cpy->setRewardsS1(s1rewards);

        std::vector<double> s0rewardsSubOpt = {0,0,0,0,0,0,5,5,0,0,0,0};
        aca2_Suboptimal_Hybrid3_cpy->setRewardsS0(s0rewardsSubOpt);
        ...
        for(int ses=0; ses < sessions; ses++)
        {
           ...
           aca2_Suboptimal_Hybrid3->setStateS0Credits(aca2_Suboptimal_Hybrid3_cpy->getS0Credits());
           drl_Suboptimal_Hybrid3->setStateS0Credits(drl_Suboptimal_Hybrid3_cpy->getS0Credits());

            
           aca2_Optimal_Hybrid3->setStateS0Credits(aca2_Optimal_Hybrid3_cpy->getS0Credits());
           aca2_Optimal_Hybrid3->setStateS1Credits(aca2_Optimal_Hybrid3_cpy->getS1Credits());
        ...
        //     drl_Optimal_Hybrid3->setRewardsS1(trueRewards_drl_Optimal_Hybrid3S1);
            ...
            aca2_Suboptimal_Hybrid3_cpy->getTrajectoryLikelihood(ratdata, ses);
            aca2_Optimal_Hybrid3_cpy->getTrajectoryLikelihood(ratdata, ses);
            drl_Suboptimal_Hybrid3_cpy->getTrajectoryLikelihood(ratdata, ses);
            drl_Optimal_Hybrid3_cpy->getTrajectoryLikelihood(ratdata, ses);
...
void testRecovery(RatData& ratdata, MazeGraph& suboptimalHybrid3, MazeGraph& optimalHybrid3, RInside &R)
{
    ...
    for(int i = start; i < end; i++)
    {
        ...
        try {
            ...
            runEMOnSimData(ratSimData, suboptimalHybrid3, optimalHybrid3, simClusterParams, false, genStrategyId,iteration);
